export const checkImage = (file) => {
  ...
  if (file.size > 1024 * 1024) err = "Dung lượng ảnh quá lớn, tối đa 1MB";
...
export const imageUpload = async (images) => {
  ...
  for (const image of images) {
    ...
    const url = `https://api.cloudinary.com/v1_1/dswg5in7u/${
      image.type.includes("video") ? "video" : "image"
    }/upload`;
    ...
    imgArr.push({
      ...
      url: data.secure_url,
      type: image.type.includes("video") ? "video" : "image",
    });
  }
  return imgArr;
};
...
export const videoUpload = async (videos) => {
  ...
  for (const video of videos) {
    ...
    const res = await fetch(
      "https://api.cloudinary.com/v1_1/dswg5in7u/video/upload",
      {
        method: "POST",
        body: formData,
      }
